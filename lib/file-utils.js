...
/*
 * Get the full path of the template file for generating diagrams based on
 * config.
 */
function getTemplatePath(templateFileName, config) {
  ...
  return path.join(__dirname, '..', 'views/diagram/', `${templateFileName}.pug`);
}

/*
 * Prepare the specified directory for saving HTML diagrams by deleting
...
/*
 * Render the HTML for a diagram based on the config.
 */
