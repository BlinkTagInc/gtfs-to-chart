...
module.exports = initialConfig => {
  ...
  return Promise.all(config.agencies.map(async agency => {
    ...
    const bar = logUtils.progressBar(`${agencyKey}: Generating ${config.outputFormat.toUpperCase()} diagrams [:bar] :current/:total`, {total: routes.length}, config);
    ...
    for (const route of routes) {
      ...
      const diagramHTML = await utils.generateDiagramHTML(route, config);

      const htmlPath = path.join(exportPath, sanitize(`${routeName}.html`));
